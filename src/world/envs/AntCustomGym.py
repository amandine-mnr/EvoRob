...
class AntCustomEnv(MujocoEnv, utils.EzPickle):
    ...
    def step(self, action):
        ...
        xy_position_before2 = self.data.body(self._main_body2).xpos[:2].copy()
        if self.body_ids is not None:
            self.apply_force()
        self.do_simulation(action, self.frame_skip)
        xy_position_after = self.data.body(self._main_body).xpos[:2].copy()
        xy_position_after2 = self.data.body(self._main_body2).xpos[:2].copy()

        xy_velocity = ((xy_position_after - xy_position_before)+(xy_position_after2 - xy_position_before2)) / self.dt 
